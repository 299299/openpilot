...
class LongitudinalMpc:
    ...
    def set_weights(self,
        ...
        if self.mode == 'acc':
            ...
            constraint_cost_weights = [
                LIMIT_COST, LIMIT_COST, LIMIT_COST, 50.0
            ]
        else:
            raise NotImplementedError(
                f'Planner mode {self.mode} not recognized in planner cost set')
    ...
    def process_lead(self, lead):
        ...
        min_x_lead = (
            (v_ego + v_lead) / 2) * (v_ego - v_lead) / (-ACCEL_MIN * 2)
    ...
    def update(self,
        ...
        self.update_params()
        t_follow = get_T_FOLLOW(personality, base_value=self.t_follow)
        v_ego = self.x0[1]
        ...
        if self.mode == 'acc':
            ...
            v_cruise_clipped = np.clip(v_cruise * np.ones(N + 1), v_lower,
                                       v_upper)
            cruise_obstacle = np.cumsum(
                T_DIFFS * v_cruise_clipped) + get_safe_obstacle_distance(
                    v_cruise_clipped,
                    t_follow,
            ...
            self.source = 'e2e' if x_and_cruise[1, 0] < x_and_cruise[
                1, 1] else 'cruise'

        else:
            raise NotImplementedError(
                f'Planner mode {self.mode} not recognized in planner update')
        ...
        self.run()
        if (np.any(lead_xv_0[FCW_IDXS, 0] -
                   self.x_sol[FCW_IDXS, 0] < CRASH_DISTANCE)
        ...
        if self.mode == 'blended':
            if any((lead_0_obstacle - get_safe_obstacle_distance(
                    self.x_sol[:, 1], t_follow,
                    stop_distance=self.stop_distance)) -
                   self.x_sol[:, 0] < 0.0):
                self.source = 'lead0'
            if any((lead_1_obstacle - get_safe_obstacle_distance(self.x_sol[:,1], t_follow, stop_distance=self.stop_distance))- self.x_sol[:,0] < 0.0) and \
